...
export async function createBankTransaction(formData: FormData) {
    ...
    const { data: tx, error: txError } = await supabase.from('bank_transactions').insert({
        organization_id: userData.organization_id,
        bank_account_id,
        date,
        description,
    ...
    // 2. Actualizar balance de la cuenta bancaria
    const { data: account } = await supabase.from('bank_accounts').select('current_balance').eq('id', bank_account_id).single()
    if (account) {
